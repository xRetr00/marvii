    ...
    match function {
        ...
        "update_voice_server_settings" => ControllerSchema {
            ...
            inputs: vec![
                ...
                FieldSchema {
                    name: "vad_onset_threshold",
                    ty: TypeSchema::Option(Box::new(TypeSchema::F64)),
                    comment: "Always-on VAD RMS onset threshold.",
                    required: false,
                },
                FieldSchema {
                    name: "vad_hangover_ms",
                    ty: TypeSchema::Option(Box::new(TypeSchema::U64)),
                    comment: "Silence duration that closes an utterance.",
                    required: false,
                },
                FieldSchema {
                    name: "vad_min_speech_ms",
                    ty: TypeSchema::Option(Box::new(TypeSchema::U64)),
                    comment: "Minimum connected speech duration to transcribe.",
                    required: false,
                },
                FieldSchema {
                    name: "vad_max_utterance_secs",
                    ty: TypeSchema::Option(Box::new(TypeSchema::F64)),
                    comment: "Maximum duration of one always-on utterance.",
                    required: false,
                },
            ],
            outputs: vec![json_output("snapshot", "Updated config snapshot.")],
